...
<!-- Loading (hidden by default - only shown when checking existing session) -->
<div id="loading" class="min-h-screen flex items-center justify-center" style="display:none">
...
<!-- ═══════════════════════════════════════════════════════════ -->
<!--   AUTH VIEW (visible by default — login form shows immediately) -->
<!-- ═══════════════════════════════════════════════════════════ -->
...
<!--   DASHBOARD VIEW                                           -->
<!-- ═══════════════════════════════════════════════════════════ -->
<div id="dashboard-view" class="min-h-screen" style="display:none">
...
function showAuthView(form){
  ...
  ['login-form','register-form','forgot-form','reset-form'].forEach(f=>{
    ...
    if(el)el.style.display=(f===form+'-form')?'':'none';
  });
}
...
(async function init(){
  ...
  if(params.get('payment')==='success'){
    if(authToken){
      ...
      await loadDashboard();
      setTimeout(()=>showToast('Payment successful! Your license will appear shortly.','success'),500);
    }
    window.history.replaceState({},document.title,'/portal');
    return;
  }
  ...
  // If user has a saved session token, check it
  if(authToken){
    // Show loading spinner while we verify the session
    document.getElementById('auth-view').style.display='none';
    document.getElementById('loading').style.display='';
    try{
      const data=await api('GET','/api/portal/me');
      ...
      authToken=null;localStorage.removeItem('portal_token');showAuthView('login');
    }
  }
  // If no token, auth-view (login form) is already visible by default — nothing to do
})();
